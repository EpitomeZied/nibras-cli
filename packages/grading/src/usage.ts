...
if (process.env.NIBRAS_AI_API_KEY) {
  console.log('🔍 Debug - Key starts with:', process.env.NIBRAS_AI_API_KEY.substring(0, 15) + '...');
} else {
  console.log('🔍 Debug - Available env keys:', Object.keys(process.env).filter(k => k.includes('NIBRAS')));
}
...
async function exampleExam() {
  const modelAnswerJSON = {
    questions: [
      ...
      },
      {
        ...
        maxScore: 2,
        modelAnswer: "No, NULL represents an unknown or missing value, which is fundamentally different from zero or an empty string.",
      },
    ],
  };
  ...
  const result = await grade({
    ...
    studentAnswers: [
      ...
      {
        questionId: "e2",
        answer: "INNER JOIN gets common records between tables, LEFT JOIN gets everything from the left table.",
      },
      {
        questionId: "e3",
      ...
      modelAnswerQuestions: [
        {
          ...
          type: "long_answer",
          maxScore: 10,
          modelAnswer: "Normalization is the process of organizing data to reduce redundancy and improve data integrity, including 1NF, 2NF, and 3NF.",
        },
        {
          id: "e2",
          ...
          type: "short_answer",
          maxScore: 5,
          modelAnswer: "INNER JOIN: only matching rows. LEFT JOIN: all rows from left table + matches from right.",
        },
        {
          id: "e3",
          question: "Does NULL equal zero?",
